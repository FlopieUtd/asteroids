...
function shipInit() {
  ship = Ship.create(screen.width >> 1, screen.height >> 1, this);
}
...
function updateShip() {
  ...
  if (ship.pos.getX() > screen.width) ship.pos.setX(0);
  else if (ship.pos.getX() < 0) ship.pos.setX(screen.width);

  if (ship.pos.getY() > screen.height) ship.pos.setY(0);
  else if (ship.pos.getY() < 0) ship.pos.setY(screen.height);
}
...
function updateBullets() {
  ...
  for (i; i > -1; --i) {
    ...
    if (b.pos.getX() > screen.width) b.blacklisted = true;
    else if (b.pos.getX() < 0) b.blacklisted = true;

    if (b.pos.getY() > screen.height) b.blacklisted = true;
...
function updateAsteroids() {
  ...
  for (i; i > -1; --i) {
    ...
    if (a.pos.getX() > screen.width + a.radius) a.pos.setX(-a.radius);
    else if (a.pos.getX() < -a.radius) a.pos.setX(screen.width + a.radius);

    if (a.pos.getY() > screen.height + a.radius) a.pos.setY(-a.radius);
...
function renderScanlines() {
  ...
  for (i; i > -1; --i) {
    ...
    context.lineTo(screen.width, i * 4);
...
function resetGame() {
  ...
  ship.pos.setXY(screen.width >> 1, screen.height >> 1);
  ship.vel.setXY(0, 0);
